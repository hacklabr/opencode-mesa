...
// node:sqlite's StatementSync.run() returns `changes: number | bigint`, which is
// wider than our hand-written RunResult (`changes: number`). The real constructor
// is structurally compatible at runtime; we cast through `unknown` to the
// adapter's expected ctor type — the same bridge driver.ts uses.
const NodeCtor = DatabaseSync as unknown as ConstructorParameters<typeof NodeDatabase>[0]
...
function mkNodeDb(opts?: { readonly?: boolean }): NodeDatabase {
  ...
  openPaths.push(path)
  const db = new NodeDatabase(NodeCtor, path, opts)
  openDbs.push(db)
  return db
}
...
describe("NodeDatabase parity (13 behaviors)", () => {
  ...
  it("readonly open blocks writes (readonly→readOnly normalization)", () => {
    ...
    const w = new NodeDatabase(NodeCtor, path)
    w.exec("CREATE TABLE t(id INTEGER)")
    w.close()

    const ro = new NodeDatabase(NodeCtor, path, { readonly: true })
    openDbs.push(ro)
