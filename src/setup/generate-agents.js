...
const PRIMARY_AGENTS_DIR = join(ROOT, "src", "agents")
...
async function writePrimaryAgents() {
  const agents = [
    {
      ...
      mode: "primary",
      sourceFile: join(PRIMARY_AGENTS_DIR, "briefing-writer.md"),
    },
    {
      filename: "gestor.md",
      name: "Gestor",
      ...
      mode: "primary",
      sourceFile: join(PRIMARY_AGENTS_DIR, "gestor.md"),
    },
  ]
  ...
  for (const agent of agents) {
    ...
    const outPath = join(AGENTS_DIR, agent.filename)
...
async function writeCatalogSubagents(personas) {
  ...
  for (const persona of personas) {
    ...
    const outPath = join(AGENTS_DIR, `${agentName}.md`)
    await writeFile(outPath, content, "utf-8")
...
async function main() {
  ...
  const existing = await readdir(AGENTS_DIR).catch(() => [])
  for (const file of existing) {
    if (file.endsWith(".md")) {
      await rm(join(AGENTS_DIR, file))
    }
  }
